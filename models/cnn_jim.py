...
class Net(nn.Module):
    def __init__(self, img_dim):
        ...
        # input_shape=(16,94,94), output_shape=[16,92,92]
        ...
        modules.append(nn.ReLU())
        # input_shape=(64,44,44), output_shape=[64,22,22]
        modules.append(nn.MaxPool2d(kernel_size=2, stride=2))
        modules.append(nn.Dropout(p=0.25))
        modules.append(nn.BatchNorm2d(64))

        # input_shape=(64,22,22), output_shape=[128,20,20]
        modules.append(nn.Conv2d(in_channels=64, out_channels=128, kernel_size=3))
        modules.append(nn.ReLU())
        # input_shape=(128,20,20), output_shape=[128,10,10]
        modules.append(nn.MaxPool2d(kernel_size=2, stride=2))
        modules.append(nn.Dropout(p=0.25))
        modules.append(nn.BatchNorm2d(128))

        # input_shape=(128,10,10), output_shape=[256,8,8]
        modules.append(nn.Conv2d(in_channels=128, out_channels=256, kernel_size=3))
        modules.append(nn.ReLU())
        # input_shape=(256,8,8), output_shape=[256,4,4]
        modules.append(nn.MaxPool2d(kernel_size=2, stride=2))


        # GlobalAveragePooling
        # input_shape=(256,4,4), output_shape=[256,1,1]
        modules.append(nn.AdaptiveAvgPool2d(1))
        modules.append(nn.BatchNorm2d(256))
        # input_shape=(256,1,1), output_shape=[1]
